...
void threadfun(GLFWwindow* window)
{
    ...
    //Shader explosionRenderer(separateRenderer::updatelist[1].shaderListing[0].c_str(), separateRenderer::updatelist[1].shaderListing[1].c_str());   // oversee destruction ;)
...
void mouse_move_callback(GLFWwindow* window, double posx, double posy)
{
    ...
    if (myMouse::pickedObjectMoveMode)
    {
        ...
        for (int i = 0; i < 2500 && !collision; ++i)
        {
            ...
            {
                glm::vec3 diff = translations[i] - (myMouse::pickedObjectOffset + translations[myMouse::pickedObjectIndex]);
                if (glm::dot(diff,diff) < 4 * radius * radius)
                {
                    collision = true;
                    return;
        ...
        translations[myMouse::pickedObjectIndex] += myMouse::pickedObjectOffset;
#endif
        return;
    }
